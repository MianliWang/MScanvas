...
direction-B prototype.** It selects no production provider. A remains viable,
...
IMPLEMENTED / VALIDATED.** The first QA build passed all four native
pairs; PR #120's shared-field label repair has a new QA build and passing browser
checks, and its focused 150% native revalidation passed. Each result retains its
own build attribution.
...
M7.4 is **IN PROGRESS**, with conversion/results/recovery/figure implementation
and controlled evidence recorded in [the M7.4 record](docs/ux/M7_4_CONVERSION_RESULTS_FIGURES.md).
Current native acceptance and protected publication remain pending. M7.5 remains
