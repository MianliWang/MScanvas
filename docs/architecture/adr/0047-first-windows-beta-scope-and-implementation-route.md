...
originally accepted M7 entry for planning only. The dated update above records
the subsequently authorized implementation state.
...
| **M7.4 — Conversion, results and figures** | ConversionPanel, roster mutation/queue service, ConversionItemJudgements, adoption and FigureSettingsFields/export renderer; M7.3 | Consolidate projections/actions; active-run Clear choices, direct quick PNG versus full Export figure/preview, narrow Rust/Tauri output-opening and guarded staging recovery; focused integration/native evidence | Real conversion/result/open/adopt/preview/export; quick Copy plot/PNG without the full figure dialog, PNG picker cancel/refusal/failure; all Clear choices/races; locked-staging recovery, output-opening failures, source/build refusals, Fail/Skip/collision, stop/quarantine, partial output and retry |
...
| --- | --- | --- |
| Complete supported task | M7.2/3/4/5 implement; M7.6 integrates | Installed real application completes import/view/convert/result/open-file-or-folder/adopt/figure-preview/figure-and-data export; stranded-staging cleanup/replan, missing/replaced output and handler-failure recovery; actionable unsupported input/build and missing-backend recovery; outputs checked and source bytes unchanged |
