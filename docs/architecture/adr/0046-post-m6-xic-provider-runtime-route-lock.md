...
| `NoUsableArrays` | The scan does not carry a usable pair of arrays — one absent, the two disagreeing in length, **or the m/z array carrying a non-finite value, which makes window membership undecidable for that scan**: a bare interval comparison would silently drop the point while an array-validating reader would refuse the scan, and both would otherwise pass the matrix. A spectrum with **no intensity array at all** is a case this repository has already measured, and it is not an m/z-domain problem |
...
a non-finite value in the m/z array; a scan declaring no retention time; a scan
