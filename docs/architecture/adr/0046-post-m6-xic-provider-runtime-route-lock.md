...
| --- | --- | --- | --- |
| `XIC_PROVIDER_ADMITTED` | A candidate has **implementable semantics and evidence for the actual source domain** — not merely a promising API or a successful compilation. **It must also be practical at the scale PX.3 observed**, judged against those observations rather than a number invented earlier | PX.5 may be *authorized*; PX.6 only after PX.5 is published and separately authorized | Does not implement anything, and does not itself authorize PX.5 |
