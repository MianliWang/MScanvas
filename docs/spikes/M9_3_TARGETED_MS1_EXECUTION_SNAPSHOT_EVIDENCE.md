...
| 3. Link under an unreadable marker, a whole marker of an unknown schema (`/2`, its own id, a gone owner), or no marker | `a_source_link_under_a_marker_that_is_not_proof_is_left_as_found` | All three `unowned`, all links intact |
...
| 6. Live or uncertain copy → retained | the same two unit tests (this process; a running child): retained. An owner that cannot be asked is covered only as the decision (`the_owner_decision_is_the_id_and_the_creation_time_together`: unknown) and the one-line mapping of unknown to retained in `classify`; no test drives a sweep through it | Live: retained |
