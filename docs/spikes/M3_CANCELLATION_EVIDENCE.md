...
Nine mutations were applied one at a time against the code head above, and each
was caught by the test written for it.
...
One of those nine survived on first application and is recorded here because the
repair is the interesting part. Dropping the refusal that sits between the
