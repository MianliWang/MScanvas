...
| WSP-007 | Clear workspace | P0 | Idle Clear releases rows; the M7.4 candidate exposes Remove non-running / Cancel and clear / Return for active work, with Rust revalidation and confirmed stop. Sources and finalized files are never deleted. Native acceptance is pending. |
...
**M7.4 candidate update (2026-09-15).** Basic/advanced views edit one typed
...
quick PNG/Copy plot and CSV/TSV remain separately reachable. Current native
acceptance and publication are pending in
