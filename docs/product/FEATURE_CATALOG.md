...
| ANA-004 | Targeted MS1 lookup (experimental) | P2 | **Implemented locally (M9.1), development runtime only.** In a saved project, on a layer whose source is one mzML file: typed targets and two parameters are resolved by Rust into a digest-named plan for review; the run is an accepted, cancellable operation in a supervised pyOpenMS 3.5.0 worker reading the held source through a same-volume hard link; outcomes are `DETECTED`, `DETECTED_AMBIGUOUS`, `SHARED`, `SUPPRESSED_BY_OVERLAP`, `NOT_DETECTED` (only on the engine's positive report over a window that held at least one MS1 spectrum with peaks) or `FAILED` with a reason; the validated rows and evidence are stored beside the project and read in bounded pages, with an evidence plot of the extracted M and M+1 points. Positive-mode centroided MS1, `[M+H]+`, at most 200 targets. Not identification or quantification; the runtime is not bundled. |
