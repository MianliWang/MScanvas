...
`feat/m6.4-visible-conversion-settings` and not on `main`: review of the head
that passed every gate found three truthfulness defects after that slice's one
bounded repair pass, so it stopped rather than merging. What the rest of this
section describes is therefore what that branch delivers, and `main` still
converts under one fixed intent with no visible control naming it. The open
findings are in `BOOTSTRAP_STATUS.md`.
...
the installation changed. A queue binds one intent when it is created, keeps it
